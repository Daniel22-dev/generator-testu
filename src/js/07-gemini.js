...
function applyKeyEnvUI(){
  ...
  if (note) note.innerHTML = 'Jsi pravděpodobně ve <strong>vestavěném prohlížeči aplikace</strong>. Klíč se i zde používá <strong>jen pro aktuální relaci</strong>; pokud by přímé generování nefungovalo správně, otevři generátor v běžném Chrome/Safari. Zdarma: <a href="https://aistudio.google.com/apikey" target="_blank" rel="noopener noreferrer">aistudio.google.com</a> → Get API key.';
}
...
function buildGeminiRequestBody(prompt, extraParts=[], opts={}) {
  ...
  const body = {
    ...
    systemInstruction: { parts:[{ text:aiTrustedSystemInstruction() }] },
    generationConfig
  };
...
function geminiApiErrorMessage(res, data, model, attempts, retryMs=0){
  ...
  if(res.status === 400 || /INVALID_ARGUMENT/i.test(apiStatus)){
    ...
    action = 'Zkontroluj Gemini API klíč na první stránce v sekci AI připojení. Od 19. 6. 2026 Google vyžaduje, aby byl klíč omezený na Gemini API — neomezené klíče vrací chybu 403. Zkontroluj nastavení klíče na aistudio.google.com → API Keys nebo vytvoř nový omezený klíč.';
  } else if(modelGone){
    why = `Zvolený model „${model}" pravděpodobně není dostupný nebo není podporovaný pro tento endpoint.`;
    action = `Změň název modelu v poli „Model" v sekci AI připojení na první stránce, např. na ${GEMINI_MODEL_DEFAULT} nebo ${GEMINI_FALLBACK_MODELS[0]}.`;
...
async function callGeminiJSON(prompt, extraParts=[], opts={}){
  ...
  if(!geminiApiKey)throw new Error('Gemini API klíč není nastaven. Zadej ho na první stránce v sekci AI připojení.');
