...
// gemini-3.5-flash je PREVIEW model s velmi malou free kvótou (RPM=5, RPD=20) —
// viz Google AI Studio → Rate limits → Gemini 3.5 Flash. gemini-2.5-flash je
// STABILNÍ model s výrazně vyššími limity a je volný na free tieru.
const GEMINI_MODEL_DEFAULT = 'gemini-2.5-flash';
// Když primární model narazí na limit (429/RESOURCE_EXHAUSTED), zkusíme JEDNOU
// automaticky záložní model. Limity Gemini free tieru jsou PER MODEL, takže
// přepnutí na jiný model = čerstvá kvóta. gemini-2.5-flash-lite má na free tieru
// nejvyšší limity a Google ho uvádí jako stabilní; gemini-flash-latest je alias
// na nejnovější Flash (nikdy ne 404). Když Google nějaký zruší, přepiš seznam tady.
...
function loadGeminiModel(){
  ...
  geminiModel = (stored && isValidModelName(stored)) ? normalizeModelName(stored) : GEMINI_MODEL_DEFAULT;
...
function geminiApiErrorMessage(res, data, model, attempts, retryMs=0){
  ...
  if(res.status === 400 || /INVALID_ARGUMENT/i.test(apiStatus)){
    ...
    action = retryMs
      ? ('Překročen limit. Generátor tlačítko dočasně vypnul; zkus to znovu nejdříve za ' + geminiFormatWait(retryMs) + '. Neklikej opakovaně, další kliknutí by zbytečně pálilo limit.')
    ...
    why = 'Dvě možné příčiny: (1) Servery Gemini jsou přetížené — stává se ve špičkách, trvá pár minut. (2) Vyčerpaný denní limit (RPD) — na Free tier je limit 20 requestů denně; po dosažení Google vrací 503 místo 429. Zkontroluj na aistudio.google.com → Rate Limit, jestli máš RPD na 20/20.';
    action = 'Pokud je RPD vyčerpaný: limit se resetuje o půlnoci UTC (kolem 1–2 hod. v noci CZ). Nebo přejdi na Pay-as-you-go (přidej kartu v AI Studiu) — limit pak bude 1000/den. Pokud RPD není vyčerpaný: počkej 2–3 minuty a zkus znovu, nebo přepni na 🪶 Lite model ve žluté sekci.';
  ...
  for(let attempt=1; attempt<=maxAttempts; attempt++){
    ...
    if(!res.ok){
      ...
      if(isQuota){
        ...
        // Limity jsou per-model, takže odlišný model = čerstvá kvóta. Spustí se jen
        // jednou (_fellBack) a jen je-li kam přepnout; lze vypnout přes opts.noFallback.
      ...
      // 503/UNAVAILABLE = přetížený sdílený pool primárního modelu. Než spálíme druhý
      // request na stejný (zahlcený) model, zkus JEDNOU záložní model — sedí v jiném,
      // typicky méně vytíženém segmentu a má vlastní per-model RPD kvótu.
      ...
      // 503 = Google overload: omezíme retry na 2 pokusy celkem, abychom nespálili denní kvótu prázdnými requesty.
