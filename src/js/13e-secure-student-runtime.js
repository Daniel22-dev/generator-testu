function buildSecureStudentHtml(publicCfg, studentVariants) {
  ...
  const formsEnabled = !!publicCfg.formsSubmissionUrl;
  const secureModeText = formsEnabled ? (S.secureModeFormsText || S.secureModeText) : S.secureModeText;
  const doneText = formsEnabled ? (S.doneFormsText || S.doneText) : S.doneText;
  const submitSecureText = formsEnabled ? (S.submitSecureForms || S.submitSecure) : S.submitSecure;
  const jokerSecureText = formsEnabled ? (S.jokerSecureFormsHint || S.jokerSecureHint) : S.jokerSecureHint;
  const formsIntro = formsEnabled ? '<div class="ok"><b>'+H(S.formsTitle || 'Google Forms')+':</b> '+H(S.formsIntro || '')+'</div>' : '';
  const formsBox = formsEnabled ? '<div class="forms-submit-box" id="formsSubmissionBox"><h2>'+H(S.formsTitle || 'Google Forms')+'</h2><div class="forms-step"><span class="forms-num">1</span><div><b>'+H(S.formsStep1 || 'Copy the submission code.')+'</b><button type="button" onclick="copySubmissionPayload()">📋 '+H(S.formsCopy || 'Copy submission code')+'</button></div></div><div class="forms-step"><span class="forms-num">2</span><div><b>'+H(S.formsStep2 || 'Open the form and submit the code.')+'</b><button type="button" class="secondary" onclick="openSubmissionForm()">🟣 '+H(S.formsOpen || 'Open form')+'</button></div></div><div id="formsSubmitStatus" class="small muted" role="status"></div></div><div id="formsPayloadWarning" class="danger hidden"></div>' : '';
  const fallbackOpen = formsEnabled ? '' : ' open';
  return '<!DOCTYPE html>\n<html lang="'+htmlLang+'"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>'+H(publicCfg.nazev)+' — student</title><style>'+secureCss()+secureStudentExtraCss()+secureStudentThemeCss(publicCfg.tema)+'</style></head><body>'+auditCommentHtml(publicCfg)+
    ...
    '<div class="warn"><b>'+H(S.secureMode)+':</b> '+H(secureModeText)+'</div>'+formsIntro+
    ...
    '<section id="done" class="card hidden"><h1>'+H(S.done)+'</h1><div class="ok">'+H(doneText)+'</div><div id="jokerDoneBox" class="joker-result hidden"></div>'+formsBox+'<details id="answersFallback" class="archive-note"'+fallbackOpen+'><summary><b>'+H(S.fallbackTitle || S.download)+'</b></summary><div class="small muted" style="margin:8px 0">'+H(S.fallbackHint || '')+'</div><div class="row"><button onclick="downloadAnswers()">'+H(S.download)+'</button><button class="secondary" onclick="shareAnswers()">'+H(S.share)+'</button><button class="ghost" onclick="copyAnswers()">'+H(S.copyBackup)+'</button></div><textarea id="answerBackup" class="backup" readonly></textarea></details><button class="ghost teacher-open" type="button" onclick="openTeacherModal()">🔒 '+H(S.teacher)+'</button></section>'+
...
function storageRemove(kind){try{localStorage.removeItem(storageKey(kind));}catch(e){}}
...
function clearSubmittedLocked(){storageRemove('submitted');}
...
async function rosterHash(value){if(!(window.crypto&&crypto.subtle&&window.TextEncoder))throw new Error(t('cryptoRequired'));var input='GIT-DIFF-ROSTER-V1|'+String(CFG.diffRosterSalt||'')+'|'+normRosterIdentity(value);var dig=await crypto.subtle.digest('SHA-256',new TextEncoder().encode(input));return b64UrlRoster(dig);}
async function identityCodeHash(value){if(!(window.crypto&&crypto.subtle&&window.TextEncoder))throw new Error(t('cryptoRequired'));var input='GIT-IDENTITY-CODE-V1|'+String(CFG.diffRosterSalt||'')+'|'+normRosterIdentity(value);var dig=await crypto.subtle.digest('SHA-256',new TextEncoder().encode(input));return b64UrlRoster(dig);}
...
async function startTest(){const name=$('studentName').value.trim();if(!name){$('studentName').focus();return;}if(submittedLocked()){showSubmittedLocked();return;}try{if(!(await identityAllowed(name))){sModal(t('invalidIdentityCode'),t('codeVerification'));$('studentName').focus();return;}}catch(err){sModal(String(err&&err.message?err.message:err),t('codeVerification'));return;}if(CFG.zolicek&&JOKER_CHOICE===null){sModal(t('jokerChoiceHint','Vyber před začátkem testu, zda píšeš test, nebo bereš žolíka.'),t('jokerChoiceTitle'));return;}if(!checkDevice(false))return;JOKER_USED=!!JOKER_CHOICE;JOKER_SELECTED_AT=JOKER_USED?new Date().toISOString():'';try{ACTIVE_KEY=await chooseVariant(name);}catch(err){sModal(String(err&&err.message?err.message:err),t('differentiatedTest'));return;}if((CFG.diffGroups||[]).length&&!ACTIVE_KEY){sModal(t('unassignedIdentity'),t('differentiatedTest'));$('studentName').focus();return;}EXS=STUDENT_VARIANTS[ACTIVE_KEY]||STUDENT_VARIANTS.__default||[];applyA11y(ACTIVE_KEY);STARTED_AT=new Date().toISOString();ATTEMPT_ID=makeAttemptId();RESP={};ANSWER_CHANGE_STATS={};LAST_RESP_SERIAL={};LAST_CHANGE_TS={};SEC_EVENTS=[{t:STARTED_AT,type:'attempt-start',detail:currentAttemptId()}];if(JOKER_USED)SEC_EVENTS.push({t:JOKER_SELECTED_AT,type:'joker-used'});LOCKED=false;SUBMITTED=false;renderTest();applyRuntimeRandomization();$('intro').classList.add('hidden');$('test').classList.remove('hidden');updateJokerUi();startTimer();startSplitMonitor();}
...
function clickOrd(id,origIdx){
  ...
  var n=texts.length;
  var picks=Array.isArray(RESP[id])?RESP[id].slice():[];
...
function updateSecureSubmitVisibility(i){var card=$('secureSubmitCard');if(!card)return;card.classList.toggle('hidden',CFG.layout==='tabs'&&Number(i)!==EXS.length-1);}
function switchExercise(i){document.querySelectorAll('.secure-tab').forEach(b=>b.classList.toggle('active',String(b.dataset.ex)===String(i)));document.querySelectorAll('.ex-panel').forEach(p=>p.classList.toggle('hidden',String(p.dataset.ex)!==String(i)));updateSecureSubmitVisibility(i);try{window.scrollTo({top:0,behavior:'smooth'});}catch(_){window.scrollTo(0,0);}}
function renderTest(){const box=$('exerciseArea');if(CFG.layout==='tabs'){const tabs='<div class="secure-tabs">'+EXS.map((ex,ei)=>'<button type="button" class="secure-tab '+(ei===0?'active':'')+'" data-ex="'+ei+'" onclick="switchExercise('+ei+')">'+esc(t('exercise','Exercise'))+' '+(ei+1)+'</button>').join('')+'</div>';const panels=EXS.map((ex,ei)=>'<div class="ex-panel '+(ei===0?'':'hidden')+'" data-ex="'+ei+'"><div class="card"><h2>'+esc(t('exercise','Exercise'))+' '+(ei+1)+': '+esc(ex.title||ex.type)+'</h2>'+renderItems(ex,ei)+'<div class="navrow">'+(ei>0?'<button type="button" class="ghost" onclick="switchExercise('+(ei-1)+')">'+esc(t('prev','Previous'))+'</button>':'')+(ei<EXS.length-1?'<button type="button" class="secondary" onclick="switchExercise('+(ei+1)+')">'+esc(t('next','Next'))+'</button>':'')+'</div></div></div>').join('');box.innerHTML=tabs+panels;updateSecureSubmitVisibility(0);}else{box.innerHTML=EXS.map((ex,ei)=>'<div class="card"><h2>'+esc(t('exercise','Exercise'))+' '+(ei+1)+': '+esc(ex.title||ex.type)+'</h2>'+renderItems(ex,ei)+'</div>').join('');updateSecureSubmitVisibility(EXS.length-1);}}
...
function safeFormsSubmissionUrl(){try{if(!CFG.formsSubmissionUrl)return '';var u=new URL(String(CFG.formsSubmissionUrl));var host=String(u.hostname||'').toLowerCase();if(u.protocol!=='https:')return '';if(host==='forms.gle'&&u.pathname&&u.pathname!=='/')return u.toString();if(host==='docs.google.com'&&/^\/forms\/(?:u\/\d+\/)?d(?:\/e)?\/[^/]+\/viewform\/?$/i.test(u.pathname||''))return u.toString();return '';}catch(_){return '';}}
function formsPayloadLimit(){var n=Number(CFG.formsPayloadSafeChars)||24000;return Math.max(4000,Math.min(30000,n));}
