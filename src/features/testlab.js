...
async function testLabPingGemini(){
  ...
  try {
    ...
    if (!genAiAvailable()) throw new Error('AI služba není dostupná.');
    var t0 = Date.now();
    var data = await callGeminiJSON('Reply with exactly: {"ok":true}', [], {operation:'diagnostic-ping'});
    var ms = Date.now() - t0;
...
function downloadE2EStarter(){
  ...
  global.GHRABGeneratorFeatures.testLab = Object.freeze({
    open: openTestLab,
