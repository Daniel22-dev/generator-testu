...
function parseStdout(result) { try { return JSON.parse(result.stdout); } catch { return null; } }

const results = [];
results.push(execute('package-selftest', [path.join(VENDOR, 'MASTER/TOOLS/package-selftest.mjs')]));
const packageReport = parseStdout(results.at(-1));
results.push({
  id: 'trusted-package-check-digest', expectedExit: 0,
...
results.push(execute('contract-selftest', [path.join(VENDOR, 'MASTER/TOOLS/contract-selftest.mjs')]));
const contractReport = parseStdout(results.at(-1));
results.push({
  id: 'g02-reference-contract', expectedExit: 0,
...
results.push(execute('generator-policy', [path.join(VENDOR, 'MASTER/TOOLS/validate-policy.mjs'), path.join(ROOT, 'security/garp27/garp-policy.json'), '--core', core, '--inventory', inventory]));
results.push(execute('live-deferred', [path.join(VENDOR, 'MASTER/TOOLS/validate-live-status.mjs'), path.join(ROOT, 'security/garp27/live-status.json'), '--profile', path.join(ROOT, 'security/garp27/application-migration-profile.json')], 3));

const failed = results.filter(r => !r.pass).length;
const report = {
  classification: 'GARP27_CONTRACT_GATE', status: failed ? 'FAIL' : 'PASS', garpVersion: '2.7',
  consolidationRevision: trust.consolidationRevision, appId: 'generator', serverPhase: 'DEFERRED_BY_OWNER_DECISION',
  results, summary: { total: results.length, passed: results.length - failed, failed }
};
