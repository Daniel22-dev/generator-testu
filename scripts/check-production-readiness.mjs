...
requireText(shell, /<body class=['"]acc-locked['"]>/, 'Aplikace neni v HTML zamcena pred spustenim JavaScriptu.');
requireText(shell, /id=['"]accessGate['"]/, 'V HTML chybi staticka fail-closed pristupova brana.');
requireText(shell, /data-access-recovery=['"]lock['"]/, 'Staticka brana nema nezavisle tlacitko pro vynuceni PINu.');
requireText(shell, /Nouzová brána v\{\{APP_VERSION\}\}/, 'Staticka brana nema viditelny verzovany marker.');
requireText(shell, /window\.__accessGateRecover\s*=\s*recover/, 'Staticka brana nema nezavisly recovery skript.');
requireText(shell, /serviceWorker\.getRegistrations/, 'Nouzova obsluha neumi odstranit stary service worker.');
requireText(shell, /caches\.keys/, 'Nouzova obsluha neumi smazat starou PWA cache.');
requireText(init, /window\.__ACCESS_INIT_REACHED__\s*=\s*true/, 'Samostatny init modul nema boot marker.');
requireText(init, /setTimeout\(runAccessBootSafely,\s*0\)/, 'Samostatny init modul nespousti pristupovou branu.');
requireText(buildScript, /data-source=\"\$\{file\}\"/, 'Build nesklada zdrojove moduly do samostatnych script tagu.');
requireText(access, /accSessionToken\(p\)[\s\S]*RELEASE\.version/, 'Session odemceni neni navazane na konkretni verzi aplikace.');
requireText(access, /reset-access/, 'Chybi servisni reset pristupoveho profilu pres URL.');
requireText(access, /searchParams\.get\(['"]lock['"]\)/, 'Chybi servisni okamzite uzamceni pres URL.');
