...
try {
  ...
  check(api?.session?.contract === 'ghrab-suite-session-v1' && typeof api?.session?.end === 'function' && typeof api?.session?.onEnd === 'function' && typeof api?.session?.acknowledge === 'function', 'runtime suite session contract');
  ...
  const previousPlatform = platformPatch > 0 ? `${platformMajor}.${platformMinor}.${platformPatch - 1}` : `${platformMajor}.${Math.max(0, platformMinor - 1)}.0`;
